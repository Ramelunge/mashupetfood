    ...
    <div style={{ padding: "4rem 0 6rem", background: "var(--color-cream)" }}>
      <div className="container-site" style={{ maxWidth: "960px" }}>
        <div style={{ textAlign: "center", marginBottom: "3rem" }}>
          ...
          <p className="section-sub">Respondemos en menos de 24 horas. Por WhatsApp incluso más rápido.</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "start" }} className="contact-grid">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            <div style={{ background: "#fff", borderRadius: "var(--radius-card)", padding: "1.5rem", boxShadow: "var(--shadow-card)" }}>
              <h3 style={{ fontWeight: 700, color: "var(--color-secondary)", marginBottom: "1rem" }}>Información de contacto</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                {cfg.email_contacto && (
                  <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                    <span style={{ fontSize: "1.25rem" }}>✉️</span>
                    <div>
                      <p style={{ fontWeight: 600, color: "var(--color-secondary)", fontSize: "0.9rem" }}>Email</p>
                      <a href={`mailto:${cfg.email_contacto}`} style={{ color: "var(--color-primary)", textDecoration: "none" }}>{cfg.email_contacto}</a>
                    </div>
                  </div>
                )}
                {cfg.whatsapp_numero && (
                  <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                    ...
                    <div>
                      <p style={{ fontWeight: 600, color: "var(--color-secondary)", fontSize: "0.9rem" }}>WhatsApp</p>
                      <a href={`https://wa.me/${cfg.whatsapp_numero}`} target="_blank" rel="noopener noreferrer" style={{ color: "var(--color-primary)", textDecoration: "none" }}>+{cfg.whatsapp_numero}</a>
                    </div>
                  </div>
                )}
                {cfg.horario && (
                  <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                    <span style={{ fontSize: "1.25rem" }}>🕐</span>
                    <div>
                      <p style={{ fontWeight: 600, color: "var(--color-secondary)", fontSize: "0.9rem" }}>Horario</p>
